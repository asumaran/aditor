...
describe('Navigation Consistency Tests', () => {
  test('navigateToFirstLine and navigateToLastLine should use same logic', () => {
    ...
    const singleFirstLineLength = singleLineStart === -1 ? singleLineContent.length : singleLineStart;
    const singleLastLineStart = singleLineContent.lastIndexOf('\n'); // -1  
    const singleLastLineStartPos = singleLastLineStart === -1 ? 0 : singleLastLineStart + 1;
    const singleLastLineLength = singleLineContent.length - singleLastLineStartPos;
    ...
    const firstLineLength = firstLineEnd === -1 ? multiLineContent.length : firstLineEnd;
    ...
    const lastLineTargetPos = lastLineStartPos + Math.min(horizontalPos, lastLineLength);
  ...
  test('should handle edge cases consistently', () => {
    ...
    const trailingLastLineStartPos = trailingLastLineStart === -1 ? 0 : trailingLastLineStart + 1; // 12  
    const trailingLastLineLength = trailingNewline.length - trailingLastLineStartPos; // 12 - 12 = 0
    ...
    const newlineLastLineLength = singleNewline.length - newlineLastLineStartPos; // 1 - 1 = 0
  ...
  test('should demonstrate consistent horizontal preservation behavior', () => {
    ...
    const barFirstLineLength = barContent.indexOf('\n') === -1 ? barContent.length : barContent.indexOf('\n');
    ...
    const fooLastLineStartPos = fooLastLineStart === -1 ? 0 : fooLastLineStart + 1; // 0
    const fooLastLineLength = fooContent.length - fooLastLineStartPos; // 3
    const fooTargetPos = fooLastLineStartPos + Math.min(horizontalPos, fooLastLineLength); // 0 + Math.min(3, 3) = 3
