...
const mockSplitContentAtCursor = (beforeContent: string, afterContent: string) => {
...
const createSplitAndCreateBlock = (callbacks: {
  ...
  return (element: HTMLElement, beforeContent: string, afterContent: string) => {
...
describe('Placeholder Flicker Prevention Tests', () => {
  ...
  describe('Splitting State Management', () => {
    test('should set splitting state to true before creating new block', () => {
      ...
      expect(onSplittingStateChange.mock.invocationCallOrder[0])
        .toBeLessThan(onCreateBlockAfter.mock.invocationCallOrder[0]);
    });
