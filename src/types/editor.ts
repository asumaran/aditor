...
export type EditorAction =
  ...
  | { type: 'UPDATE_BLOCK_REQUIRED'; payload: { id: number; required: boolean } }
  | { type: 'UPDATE_BLOCK_OPTIONS'; payload: { id: number; options: readonly Option[] } }
  | { type: 'ADD_OPTION'; payload: { blockId: number; option: Option } }
  | { type: 'REMOVE_OPTION'; payload: { blockId: number; optionId: number } }
  | { type: 'UPDATE_OPTION'; payload: { blockId: number; optionId: number; text: string } }
